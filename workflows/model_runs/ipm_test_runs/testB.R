...
# Specify the model
model_file <- "models//ipm//tests//21june2024_testB.txt"
save_file <- "results//ipm//tests//21june2024_rsltB.rds"
...
# Update p_cjs for the test
p_cjs <- ipm_data$p_cjs
p_cjs[,3] <- p_cjs[,3] + 1
p_cjs <- rbind(p_cjs, ipm_data$p_cjs[1:32,])
p_cjs[64:nrow(p_cjs), 6] <- colSums(ipm_data$calf)[-1]
