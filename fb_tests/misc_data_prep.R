...
index_available_yr <- as.numeric(names(nf_ch)[index_available])
fac <- nf_ch[index_available]
mac <- nm_ch[index_available] 
cc <- ncf_ch[index_available] + ncm_ch[index_available]
fp <- cjs_rslt$pf_logit %>%
  filter(yr %in% index_available_yr) %>%
  pull(mn) %>%
  ilogit()
mp <- cjs_rslt$pm_logit %>%
  filter(yr %in% index_available_yr) %>%
  pull(mn) %>%
  ilogit()
nr <- length(fac)
...
saveRDS(dtlist, "fb_tests//misc_data.rds")
