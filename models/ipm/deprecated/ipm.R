{
  ...
  for(t in 2:nyr){ #t (year)
    ...
    N[1,1,t] ~ dnorm(Ne[1,1,t], 1/Ne[1,1,yr])T(Nc_obs[yr], )
    N[1,2,t] ~ N[1,1,t]
  ...
  for(t in 1:nyr){ #t
    R0[yr]    ~  dnorm(Rb0, R_tau)
    RD[t]     <- Rdd * ((Nt[t] - mean(Nt[])) / sd(Nt[]))
    log(R[t]) <- R0 + RD[t]
  }#t
  ...
  for(t in 1:nyr){
    Sc[yr] ~ dnorm(Sc0, Sc_tau)
    Sf[yr] ~ dnorm(Sf0, Sf_tau)
    Sm[yr] ~ dnorm(Sm0, Sm_tau)
