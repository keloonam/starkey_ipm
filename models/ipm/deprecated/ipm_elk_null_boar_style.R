ipm_code <- nimbleCode({
  ...
  R_B0_prm_ad ~ logit(dunif(0, 1))
  R_B0_yng_ad ~ logit(dunif(0, 1))
  sd_R ~ dunif(0, 50)

  for(yr in 1:n_year){ # yr
    R_yr[yr] ~ dnorm(0, sd = sd_R)
    logit(R[1, yr]) <- 0 # calves do not breed
    logit(R[2, yr]) <- 0 # subadults do not breed
    logit(R[3, yr]) <- R_B0_yng_ad + R_yr[yr] # young adults and prime adults
    logit(R[4, yr]) <- R_B0_prm_ad + R_yr[yr] # reproduce at different rates
  } # yr
  ...
  S_C_B0   ~ logit(dunif(0, 1))
  S_Y_F_B0 ~ logit(dunif(0, 1))
  S_Y_M_B0 ~ logit(dunif(0, 1))
  S_A_F_B0 ~ logit(dunif(0, 1))
  S_A_M_B0 ~ logit(dunif(0, 1))
  ...
  for(yr in 1:n_year){ # yr
    ...
    S_Y_F_yr[yr] ~ dnorm(0, sd = sd_S_Y_F)
    S_Y_M_yr[yr] ~ dnorm(0, sd = sd_S_Y_M)
    logit(S[3,1,yr]) <- S_Y_F_B0 + S_Y_F_yr[yr]
    logit(S[3,2,yr]) <- S_Y_M_B0 + S_Y_M_yr[yr]
    
    # Survival from 2.5+ (to age class 4)
    # Separate random effects by year for both males and females
    S_A_F_yr[yr] ~ dnorm(0, sd = sd_S_A_F)
    S_A_M_yr[yr] ~ dnorm(0, sd = sd_S_A_M)
  ...
  ##### Abundance -- starting
  # can adjust this later on to take opinions on initial population size as
    # data to make informative priors
  
  for(x in 1:2){ # x
    for(a in 1:4){ # a
      init_N[a,x,1] ~ T(dnorm(100, sd = 100), 1, Inf)
      N[a,x,1] ~ round(init_N[a,x,1])
    } # a
  } # x
  ...
  for(yr in 2:n_year){ # yr
    ...
    N_3_ca[yr] ~ dpois(R[3,yr] * N[3,1,yr-1] * 0.5)
    N_4_ca[yr] ~ dpois(R[4,yr] * N[4,1,yr-1] * 0.5)
    N[1,1,yr] <- N_3_ca[yr] + N_4_ca[yr]
    N[1,2,yr] <- N[1,1,yr]
    R_EST[yr] <- (R[3,yr]*N[3,1,yr-1] + R[4,yr]*N[4,1,yr-1]) / N_f[yr-1]
    
    for(x in 1:2){ # x (sex)
      ##### Survival 
      # 3 stages of survival: Removals, Natural and Harvest
        # S_R: p(not being removed from the main herd from feedgrounds)
        # S_N: p(surviving from natural mortality ~Nov 1 to hunting season)
        # S_H: p(surviving harvest season)
      ...
      n_ad_rem[x,yr] <- sum(N[2:4,x,yr-1]) - sum(N_JAN[2:4],x,yr)
      n_ca_rem[x,yr] <- N[1,x,yr-1] - N_JAN[1,x,yr]
      
      for(a in 1:4){
        N_JAN[a,x,yr] ~ dbinom(S_R[a,x,yr], N[a,x,yr-1])
        N_AUG[a,x,yr] ~ dbinom(S_N[a,x,yr], N_JAN[a,x,yr])
        N_NOV[a,x,yr] ~ dbinom(S_H[a,x,yr], N_AUG[a,s,yr])
        n_harvested[a,x,yr] <- N_AUG[a,x,yr] - N_NOV[a,x,yr]
      } # a (age)
      
      # age transitions and herd additions happen here
      N[2,x,yr] <- N_NOV[1,x,yr] + n_ca_add[x,yr]
      N[3,x,yr] <- N_NOV[2,x,yr] # assume additions are full adults to simplify
      N[4,x,yr] <- N_NOV[3,x,yr] + N_NOV[4,x,yr] + n_ad_add[x,yr]
    } # x (sex)
    ...
    N_m[1]     <- N[3,2,yr] + N[4,2,yr] + N[2,2,yr]
    N_tot[yr]  <- N_af[yr]  + N_am[yr]  + N_ca[yr] + N_ym[yr] + N_yf[yr]
  } # yr
  ...
  p0_c_fg   <- logit(dunif(0,1))
  p0_f_fg   <- logit(dunif(0,1))
  p0_m_fg   <- logit(dunif(0,1))
  p_c_fg_sd <- dunif(0, 50)
  p_f_fg_sd <- dunif(0, 50)
  p_m_fg_sd <- dunif(0, 50)
  for(yr in 1:n_year){
    logit(P_FG_C[yr]) <- dnorm(p0_c_fg, sd = p_c_fg_sd)
    logit(P_FG_F[yr]) <- dnorm(p0_f_fg, sd = p_f_fg_sd)
    logit(P_FG_M[yr]) <- dnorm(p0_m_fg, sd = p_m_fg_sd)
  }
  for(i in 1:nn_fg_ca){
  ...
  # Ratio - Feedgrounds
  for(i in 1:nr){
    r_ratio[i,4] ~ dnorm(R_EST[r_ratio[i,1]], r_ratio[i,5])
  }
})