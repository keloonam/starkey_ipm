nimble_code <- nimbleCode({
  ...
  for(t in 2:n_year){
    ...
    NCaug[t] ~ T(dnorm(NCSe_mean[t], sd = NCSe_sd[t]), NCmin[t], 1000)
    NFaug[t] ~ T(dnorm(NFSe_mean[t], sd = NFSe_sd[t]), NFmin[t], 1000)
    NMaug[t] ~ T(dnorm(NMSe_mean[t], sd = NMSe_sd[t]), NMmin[t], 1000)
  ...
  ##### Counts #####
  sd_afcount ~ T(dnorm(0, sd = 100), 0, )
  for(i in 1:nn_fc){
    af_count[i] ~ T(dnorm(NFaug[af_count_t[i]], sd = sd_afcount), 0, )
  }
  
  sd_amcount ~ T(dnorm(0, sd = 100), 0, )
  for(i in 1:nn_mc){
